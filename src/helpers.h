...
double calculate_speed(vector<double> value){
  double speed = accumulate(value.begin(), value.end(), 0.0)/ value.size();
  return speed;
}
...
vector<double> calculate_final_kinematics(vector<double> kinematics, double acc, double T){
  ...
  double final_velocity = kinematics[1] + (acc - kinematics[2])*T;
  ...
  return {final_position, final_velocity, final_acc};
}
